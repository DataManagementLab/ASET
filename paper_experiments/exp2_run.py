...
def do_evaluation_run(
    ...
    for column_name, attribute_name in zip(column_names, dataset.ATTRIBUTES):
        ...
        # of the ones that are extracted
        num_mentioned = 0
        ...
        for document, row in zip(documents, aset_matching_stage.rows):
            ...
            if document["evaluation"][Extractor.extractor_str]["mentions"][attribute_name]:
                num_extracted += 1
                ...
                indices = document["evaluation"][Extractor.extractor_str]["mentions_diff_value"][attribute_name]
                extractions = [document["evaluation"][Extractor.extractor_str]["all_extractions"][i] for i in indices]
                valid_mentions_diff_value = [extraction["mention"] for extraction in extractions]

                if found_extraction is None:
                    num_empty += 1
                else:
                    if found_extraction.mention in valid_mentions:
                        num_correct += 1
                    elif found_extraction.mention in valid_mentions_diff_value:
                        num_diff_value += 1
                    else:
                        num_incorrect += 1

        # compute the evaluation metrics
        if num_extracted == 0:  # has never been extracted
            recall = 0
            precision = 0
            f1_score = 0
            recall_diff_value = 0
            precision_diff_value = 0
            f1_score_diff_value = 0
        else:
            recall = num_correct / num_extracted
            if num_correct + num_diff_value + num_incorrect == 0:  # no match found at all
                precision = 1
            else:
                precision = num_correct / (num_correct + num_diff_value + num_incorrect)
            if recall == 0 and precision == 0:
                f1_score = 0
            else:
                f1_score = 2 * precision * recall / (precision + recall)

            recall_diff_value = (num_correct + num_diff_value) / num_extracted
            if num_correct + num_diff_value + num_incorrect == 0:  # no match found at all
                precision_diff_value = 1
            else:
                precision_diff_value = (num_correct + num_diff_value) / (num_correct + num_diff_value + num_incorrect)
            if recall_diff_value == 0 and precision_diff_value == 0:
                f1_score_diff_value = 0
            else:
                f1_score_diff_value = 2 * precision_diff_value * recall_diff_value / (precision_diff_value + recall_diff_value)

        recalls[attribute_name] = recall
        precisions[attribute_name] = precision
        f1_scores[attribute_name] = f1_score
        recalls_diff_value[attribute_name] = recall_diff_value
        precisions_diff_value[attribute_name] = precision_diff_value
        f1_scores_diff_value[attribute_name] = f1_score_diff_value
